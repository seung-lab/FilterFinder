...
def model(img, tmp, kernel_shape, identity = False):
    ...
    source_alpha = tf.nn.sigmoid(source_alpha+bias_1)

    template_alpha = fftconvolve2d(tmp, kernel, 'VALID')
    template_alpha = tf.nn.sigmoid(template_alpha+bias_1)

    # Second Layer
    source_alpha = fftconvolve2d(source_alpha, kernel_2, 'VALID')
    source_alpha = tf.nn.sigmoid(source_alpha+bias_2)

    template_alpha = fftconvolve2d(template_alpha, kernel_2, 'VALID')
    template_alpha = tf.nn.sigmoid(template_alpha+bias_2)
...
def loss(p, radius, eps= 0.001, ltype='diff'):
    ...
    #p_max_2 = tf.reduce_sum(tf.multiply(p_2,soft_mask_p))
...
def evaluate(deterministic, source_shape, template_shape, aligned=True):
    sum_dist = 0
    if aligned:
